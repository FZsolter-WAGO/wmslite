class renderclass {
    ...
    _wmsConfig(box) {
        ...
        // Title
        tmpHTML += `<div class="d-flex gap-2 w-100 justify-content-between">`
        tmpHTML += `<h3 class="mb-4">WMS MQTT Connection</h3>`
        tmpHTML += `</div>`
        // Fancy box
        tmpHTML += `<div class="list-group w-auto">`
        tmpHTML += `
        ...
        tmpHTML += `<div class="input-group mb-1">
                        <span class="input-group-text wms_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Client ID</span>
                        <input type="text" value="${config.virtualMeters.clientId}" class="form-control wms_color_${config.darkMode?'dark':'light'}_4" id="clientId">
                    </div>`
        tmpHTML += `<div class="input-group mb-1">
        ...
        box.querySelectorAll("#clientId").forEach(input => {
            input.addEventListener("change",(event)=>{
                config.virtualMeters.clientId = event.target.value
                this._wmsConfig(content_box.querySelector("#wmsConfig_box"))
        })})
