async function loadConfigJSON() {
	let response1 = await fetch('php/ordernumber.php')
	...
	switch (orderNumber) {
		...
		case "750-8211":
			data.limits.plcModules = 20
			data.limits.rtuMeters = 0
			data.limits.tcpMeters = 30
			data.limits.rtdChannels = 30
			data.limits.digitalInputs = 30
			data.limits.virtualMeters = 30
			break;
		case "750-8212":
			data.limits.plcModules = 20
			data.limits.rtuMeters = 32
		...
		case "750-8217":
			...
			break;
		case "751-9301":
			data.limits.plcModules = 0
			data.limits.rtuMeters = 32
			data.limits.tcpMeters = 10
			data.limits.rtdChannels = 2
			data.limits.digitalInputs = 8
			data.limits.virtualMeters = 10
			break;
		case "752-8303/8000-0002":
			data.limits.plcModules = 0
			data.limits.rtuMeters = 32
			data.limits.tcpMeters = 40
			data.limits.rtdChannels = 0
			data.limits.digitalInputs = 4
			data.limits.virtualMeters = 40
			break;
		default:
			data.limits.plcModules = 999
			data.limits.rtuMeters = 999
			data.limits.tcpMeters = 999
			data.limits.rtdChannels = 999
			data.limits.digitalInputs = 999
			data.limits.virtualMeters = 999
			break;
	}
	try {
...
function downloadConfig(name,fromFs) {
	if (fromFs) {
		loadConfigJSON()
		.then((target)=>{
			var button = document.createElement('a');
			button.setAttribute('href','data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify(target)))
		...
		var button = document.createElement('a');
		button.setAttribute('href','data:text/plain;charset=utf-8,' + encodeURIComponent(JSON.stringify(config)))
		button.setAttribute('download', name)
		document.body.appendChild(button)
		button.click()
		document.body.removeChild(button)
...
async function saveConfig(password,config) {
	...
	fetch('php/saveconfig.php',{
		...
		method:'POST',
		body:JSON.stringify({"password":password,"json":config,"csv":createCsv(config)})
	})
