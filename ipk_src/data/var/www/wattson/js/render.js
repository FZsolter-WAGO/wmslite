class renderclass {
    ...
    physical() {
        ...
        if (config.limits.plcModules > 0) this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="plcModules_box"></div>`
        if (config.limits.rtuMeters > 0) this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="rtuMeters_box"></div>`
        if (config.limits.tcpMeters > 0) this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="tcpMeters_box"></div>`
        if (config.limits.rtdChannels > 0) this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="rtdChannels_box"></div>`
        if (config.limits.digitalInputs > 0) this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="digitalInputs_box"></div>`
    ...
    wattson() {
        this.changeTheme(config.darkMode?'dark':'light')
        this.title_box.innerHTML = `<h1>WattsON Meters</h1>`
        this.content_box.innerHTML = ''
        if (config.limits.virtualMeters > 0) this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="wattsonConfig_box"></div>`
        if (config.limits.virtualMeters > 0) this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="wattsonMeters_box"></div>`
        if (config.limits.virtualMeters > 0) this._wattsonConfig(content_box.querySelector("#wattsonConfig_box"))
        if (config.limits.virtualMeters > 0) this._wattsonMeters(content_box.querySelector("#wattsonMeters_box"))
    }
    backupRestore() {
        this.changeTheme(config.darkMode?'dark':'light')
        this.title_box.innerHTML = `<h1>Backup/Restore Configuration</h1>`
        this.content_box.innerHTML = ''
        this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="backup_box"></div>`
        this.content_box.innerHTML += `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-4" id="restore_box"></div>`
        this._backup(content_box.querySelector("#backup_box"))
        this._restore(content_box.querySelector("#restore_box"))
    }
    loading() {
        this.changeTheme(config.darkMode?'dark':'light')
        this.title_box.innerHTML = `<h1>WattsONLite Configurator</h1><h2>Loading, please wait...</h2>`
        this.content_box.innerHTML = ``
    }
    _plcModules(box) {
        let tmpHTML = `<div class="wattson_color_${config.darkMode?'dark':'light'}_1 p-4 rounded mb-2">`
        // Title
        ...
        config.physicalMeters.plcModules.meters.forEach(meter => {
            tmpHTML += `
            <div class="list-group-item d-flex gap-3 py-3 wattson_color_${config.darkMode?'dark':'light'}_2">
            ...
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Status</span>
                            <input checked type="checkbox" class="btn-check">
                            <label class="btn btn-${((meter.enabled)?'primary':'secondary')} wattson_color_${config.darkMode?'dark':'light'}_1" for="btn-check" style="min-width:150px" id="enabled">${((meter.enabled)?'Enabled':'Disabled')}</label>
                        </div>`
            if (meter.type === "ac") {
                tmpHTML += `<div class="input-group mb-1">
                                <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Name</span>
                                <input type="text" value="${meter.name}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="name">
                            </div>`
                tmpHTML += `<div class="input-group mb-1">
                                <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">AC Position</span>
                                <input type="number" min="1" max="20" value="${meter.address}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="address">
                            </div>`
            } else {
                tmpHTML += `<div class="input-group mb-1">
                                <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Channel 1 Name</span>
                                <input type="text" value="${meter.name}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="name">
                            </div>`
                tmpHTML += `<div class="input-group mb-1">
                                <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Channel 2 Name</span>
                                <input type="text" value="${meter.name2}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="name2">
                            </div>`
                tmpHTML += `<div class="input-group mb-1">
                                <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">DC Position</span>
                                <input type="number" min="1" max="40" value="${meter.address}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="address">
                            </div>`
            }
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Type of module</span>
                            <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="type">
    ...
    _rtuMeters(box) {
        let tmpHTML = `<div class="wattson_color_${config.darkMode?'dark':'light'}_ p-4 rounded mb-2">`
        // Title
        tmpHTML += `<div class="d-flex gap-2 w-100 justify-content-between">`
        ...
        tmpHTML += `<div class="list-group-item d-flex gap-3 py-3 wattson_color_${config.darkMode?'dark':'light'}_2"><div class="d-flex gap-2 w-100 justify-content-between"><div class="flex-fill">`
        tmpHTML += `<div class="input-group mb-1">
                    <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Baudrate</span>
                    <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" style="width:150px" id="baudrate">
        ...
        tmpHTML += `<div class="input-group mb-1">
                    <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Parity</span>
                    <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" style="width:150px" id="parity">
        ...
        tmpHTML += `<div class="input-group mb-1">
                    <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Stopbits</span>
                    <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" style="width:150px" id="stopbits">
        ...
        config.physicalMeters.rtuMeters.meters.forEach(meter => {
            ...
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Slave ID</span>
                            <input type="number" min="1" max="32" value="${meter.address}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="address">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Type of module</span>
                            <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="type">
    ...
    _tcpMeters(box) {
        ...
        config.physicalMeters.tcpMeters.gateways.forEach(gateway => {
            ...
            tmpHTML += `
            <div class="list-group-item gap-3 py-3 wattson_color_${config.darkMode?'dark':'light'}_2">
            ...
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Status</span>
                            <input checked type="checkbox" class="btn-check">
                            <label class="btn btn-${((gateway.enabled)?'primary':'secondary')} wattson_color_${config.darkMode?'dark':'light'}_1" for="btn-check" style="min-width:150px" id="enabled_gw">${((gateway.enabled)?'Enabled':'Disabled')}</label>
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Name</span>
                            <input type="text" value="${gateway.name}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="name_gw">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Address</span>
                            <input type="text" value="${gateway.address}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="address_gw">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">TCP Port</span>
                            <input type="number" min="0" max="65535" value="${gateway.port}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="port_gw">
                        </div>`
            tmpHTML += `</div>`
            ...
            gateway.meters.forEach(meter => {
                tmpHTML += `
                <div class="list-group-item d-flex gap-3 py-3 wattson_color_${config.darkMode?'dark':'light'}_3">
                    <div class="d-flex gap-2 w-100 justify-content-between" uuid=${meter.uuid}>
                        <div class="flex-fill" uuid=${meter.uuid}>`
                tmpHTML += `<h6 class="mb-2"><u>${meter.name}</u></h6>`
                tmpHTML += `<div class="input-group mb-1">
    ...
    _rtdChannels(box) {
        ...
        config.physicalMeters.rtdChannels.meters.forEach(meter => {
            ...
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Channel</span>
                            <input type="number" min="1" max="30" value="${meter.address}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="address">
                        </div>`
            tmpHTML += `</div>`
            tmpHTML += `<button style="min-width:100px" class="btn btn-danger ml-auto align-self-center mx-2" id="delete_button">Delete</button>`
            tmpHTML +=`
    ...
    _wattsonConfig(box) {
        ...
        // Title
        tmpHTML += `<div class="d-flex gap-2 w-100 justify-content-between">`
        tmpHTML += `<h3 class="mb-4">WattsON Connection</h3>`
        tmpHTML += `</div>`
        // Fancy box
        tmpHTML += `<div class="list-group w-auto">`
        tmpHTML += `
        ...
        tmpHTML += `<div class="input-group mb-1">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Status</span>
                        <input checked type="checkbox" class="btn-check">
                        <label class="btn btn-${((config.virtualMeters.enabled)?'primary':'secondary')} wattson_color_${config.darkMode?'dark':'light'}_1" for="btn-check" style="min-width:150px" id="enabled">${((config.virtualMeters.enabled)?'Enabled':'Disabled')}</label>
                    </div>`
        tmpHTML += `<div class="input-group mb-1">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Host</span>
                        <input type="text" value="${config.virtualMeters.host}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="host">
                    </div>`
        tmpHTML += `<div class="input-group mb-1">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Password</span>
                        <input type="password" value="${config.virtualMeters.password}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="password">
                    </div>`
        tmpHTML += `<div class="input-group mb-1">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Data transfer interval</span>
                        <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="onlyPriority">
        ...
        tmpHTML += `<div class="input-group mb-1">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Reboot on error</span>
                        <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="rebootOnError">
        ...
        tmpHTML += `<div class="input-group mb-1">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >SSL Mode</span>
                        <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="secure">
        ...
        if (config.virtualMeters.secure) {
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">CA Cert</span>
                            <input type="text" value="${config.virtualMeters.sslconf.caCert}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="caCert">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">CA Path</span>
                            <input type="text" value="${config.virtualMeters.sslconf.caPath}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="caPath">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Verify Peer</span>
                            <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="verifyPeer">
            ...
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Verify Host</span>
                            <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="verifyHost">
            ...
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Client Cert</span>
                            <input type="text" value="${config.virtualMeters.sslconf.clientCert}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="clientCert">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Client Cert Key</span>
                            <input type="text" value="${config.virtualMeters.sslconf.clientCertKey}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="clientCertKey">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">Client Cert Key Password</span>
                            <input type="text" value="${config.virtualMeters.sslconf.clientCertKeyPasswd}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="clientCertKeyPasswd">
                        </div>`
        }
        tmpHTML += `</div>`
        tmpHTML +=`
        ...
        box.querySelectorAll("#enabled").forEach(input => {
            input.addEventListener("click",()=>{
                ...
                this._wattsonConfig(content_box.querySelector("#wattsonConfig_box"))
        })})
    ...
    _wattsonMeters(box) {
        ...
        // Title
        tmpHTML += `<div class="d-flex gap-2 w-100 justify-content-between">`
        tmpHTML += `<h3 class="mb-4">WattsON Meters (${config.limits.virtualMeters} Meters)</h3>`
        tmpHTML += `<button style="min-width:100px" class="btn btn-success ml-auto align-self-center mx-4" id="add_button">Add Module</button>`
        tmpHTML += `</div>`
        // Fancy box
        tmpHTML += `<div class="list-group w-auto">`
        config.virtualMeters.meters.forEach(meter => {
            ...
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px">UUID</span>
                            <input type="text" value="${meter.wattson_uuid}" class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" id="wattson_uuid">
                        </div>`
            tmpHTML += `<div class="input-group mb-1">
                            <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Type of module</span>
                            <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="type">
            ...
            switch (meter.type) {
                case "plcModules":
                    if (config.physicalMeters.plcModules.meters.length > 0) {
                        tmpHTML += `<div class="input-group mb-1">
                                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Selected Meter</span>
                                        <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="meter">`
                        ...
                        tmpHTML += `<div class="input-group mb-1">
                                        <span class="form-control wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >No meter declared</span>
                                    </div>`
                    }
                    break;
                ...
                case "tcpMeters":
                    if (config.physicalMeters.tcpMeters.gateways.length > 0) {
                        tmpHTML += `<div class="input-group mb-1">
                                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >Selected Gateway</span>
                                        <select class="form-select wattson_color_${config.darkMode?'dark':'light'}_4" id="gateway">`
                        ...
                        tmpHTML += `<div class="input-group mb-1">
                                        <span class="form-control wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >No gateway declared</span>
                                    </div>`
                        tmpHTML += `<div class="input-group mb-1">
                                        <span class="form-control wattson_color_${config.darkMode?'dark':'light'}_1" style="min-width:150px" >No meter declared</span>
                                    </div>`
        ...
        box.querySelectorAll("#enabled").forEach(input => {
            input.addEventListener("click",(event)=>{
                ...
                this._wattsonMeters(content_box.querySelector("#wattsonMeters_box"))
        })})
        box.querySelector("#add_button").addEventListener("click",()=>{
            let uuid = newUUID()
        ...
        box.querySelectorAll("#wattson_uuid").forEach(input => {
            input.addEventListener("change",(event)=>{
                config.virtualMeters.meters[config.virtualMeters.meters.indexOf(config.virtualMeters.meters.find(meter => meter.uuid === event.target.parentElement.parentElement.getAttribute("uuid")))].wattson_uuid = event.target.value
                this._wattsonMeters(content_box.querySelector("#wattsonMeters_box"))
        })})
        box.querySelectorAll("#type").forEach(input => {
            input.addEventListener("change",(event)=>{
    ...
    _backup(box) {
        ...
        tmpHTML += `<div class="input-group mb-3">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_2" style="min-width:200px">Download from browser</span>
        ...
        tmpHTML += `<div class="input-group mb-0">
                        <span class="input-group-text wattson_color_${config.darkMode?'dark':'light'}_2" style="min-width:200px">Download from filesystem</span>
        ...
        box.querySelectorAll("#backupbrowser_button").forEach(button => {
            button.addEventListener("click",()=>{
                const fileName = `Browser-${window.location.host}-${new Date().toLocaleString().replace(". ","-").replace(". ","-").replace(". ","_").replaceAll(":","-")}-wattson.config.json`
                downloadConfig(fileName,false)
                this._backup(content_box.querySelector("#backup_box"))
        })})
        box.querySelectorAll("#backupfilesystem_button").forEach(button => {
            button.addEventListener("click",()=>{
                const fileName = `Filesystem-${window.location.host}-${new Date().toLocaleString().replace(". ","-").replace(". ","-").replace(". ","_").replaceAll(":","-")}-wattson.config.json`
    ...
    _restore(box) {
        ...
        tmpHTML += `</div>
                        <div class="input-group mb-3">
                            <input class="form-control wattson_color_${config.darkMode?'dark':'light'}_4" type="file" id="restoreFile" name="restoreFile" required="required">
        ...
        switch (theme) {
            case "dark":
                for (let index = 0; index < 5; index++) {
                    document.querySelectorAll(`.wattson_color_light_${index}`).forEach(element => {
                        element.classList.remove(`wattson_color_light_${index}`)
                        element.classList.add(`wattson_color_dark_${index}`)
                    });
                }
            ...
            case "light":
                for (let index = 0; index < 5; index++) {
                    document.querySelectorAll(`.wattson_color_dark_${index}`).forEach(element => {
                        element.classList.remove(`wattson_color_dark_${index}`)
                        element.classList.add(`wattson_color_light_${index}`)
                    });
                }
    ...
    initCssRule(selector,style) {
        for (let i = 0; i < document.styleSheets.length; i++) {
            ...
            if (styleSheet.ownerNode.id == "wattson_color") {
